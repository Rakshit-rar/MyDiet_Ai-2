...
st.markdown("""
<style>
/* Main container background - subtle contrast */
[data-testid="stAppViewContainer"] {
    background-image: linear-gradient(180deg, rgba(240,242,246,0.1) 0%, rgba(240,242,246,0.1) 100%);
}

/* Force cards to have a slight background to separate from main page */
.stMetric, .stAlert, div.stButton > button {
    background-color: rgba(151, 154, 158, 0.1);
    border-radius: 10px;
    padding: 10px;
}

/* Adaptive text - uses CSS variables provided by Streamlit */
/* This ensures visibility in both Light and Dark modes */
h1, h2, h3, p, span, .stMarkdown {
    color: var(--text-color);
}

/* Special styling for the generated results to make them pop */
.stMetric [data-testid="stMetricValue"] {
    color: #FF4B4B !important; /* Streamlit Red for focus */
}
</style>
